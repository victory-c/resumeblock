...
test("rejects templates with direct file access primitives", () => {
  assert.equal(hasUnsafeLatexPrimitives("\\input{/etc/passwd}"), true)
  assert.equal(hasUnsafeLatexPrimitives("\\openout\=file"), true)
  assert.equal(hasUnsafeLatexPrimitives("\\documentclass{article}\n%%HEADER%%"), false)
})
